			...
			//戦車演算
			{
				...
				for (auto& f : veh.Gun_) {
					...
					if (f.gun_info.frame3.first >= 0) {
						veh.obj.SetFrameLocalMatrix(f.gun_info.frame3.first, VECTOR_ref(VGet(0.f, 0.f, f.fired*0.5f)).Mtrans()*f.gun_info.frame3.second.Mtrans());//リコイル
						veh.col.SetFrameLocalMatrix(f.gun_info.frame3.first, VECTOR_ref(VGet(0.f, 0.f, f.fired*0.5f)).Mtrans()*f.gun_info.frame3.second.Mtrans());//リコイル
					}
				}
				//転輪
				{

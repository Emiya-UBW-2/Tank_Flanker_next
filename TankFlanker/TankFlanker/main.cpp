...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd) {
	...
	//キャラ選択
	chara.resize(1);
	UIparts->select_window(&chara[0], &Vehicles);
	//マップ読み込み
	...
	auto draw_on_shadow = [&mapparts, &carrier, &chara, &ads, &tree, &campos] {
		//マップ
	...
	while (ProcessMessage() == 0) {
				...
				//
				{
					...
					if (c.p_anime_geardown.second >= 0.5f) {
						for (auto& w : veh.use_veh.wheelframe) {
							...
							{
								auto hp = mapparts->map_col_line(tmp + (veh.mat.yvec() * (0.98f)), tmp, 0);
								if (hp.HitFlag == TRUE) {
							...
							{
								auto hp = carrier_col.CollCheck_Line(tmp + veh.mat.yvec() * (0.98f), tmp, 0, 0);
								if (hp.HitFlag == TRUE) {
									veh.add = (VECTOR_ref(hp.HitPosition) - tmp);
									{
										auto x_vec = veh.mat.xvec();
		...
		for (auto& c : chara) {
			auto& veh = c.vehicle[0];
			b2Vec2 tmpb2 = b2Vec2(
				(M_GR / fps) * (veh.mat.zvec().dot(VGet(0, 1.f, 0))),
				(M_GR / fps) * (veh.mat.yvec().dot(VGet(0, 1.f, 0)))
			);
			int z = 0;
			for (auto& f : c.foot) {
				size_t i = 0;
				...
				if (f.Foot.size() != 0) {
					for (auto& w : veh.use_veh.wheelframe) {
						...
						if (vects.x() * ((z == 0) ? 1 : -1) > 0) {
							f.Wheel[i++].body->SetTransform(b2Vec2(vects.z(), vects.y()), (z == 0) ? c.vehicle[0].wheel_Left : c.vehicle[0].wheel_Right);
						}
					}
					i = 0;
					for (auto& w : veh.use_veh.wheelframe_nospring) {
						...
						if (vects.x() * ((z == 0) ? 1 : -1) > 0) {
							f.Yudo[i++].body->SetTransform(b2Vec2(vects.z(), vects.y()), (z == 0) ? c.vehicle[0].wheel_Left : c.vehicle[0].wheel_Right);
						}
					}
					for (auto& t : f.Foot) {

...
public:
	...
	static void set_ammos(std::vector<hit::Ammos>* Ammos) {
		...
		a.resize(a.size() + 1);
		a.back().name       = "AIM9-X";
		a.back().type       = 2; //ap=0,he=1
		a.back().caliber    = 0.127f;
		a.back().penetration= 120.f;
		a.back().speed      = 600.f;
		a.back().damage     = 80;
	}
	//戦車
	...
	public:
		...
		void into(const Tanks& t) {
			...
			this->max_speed_limit= t.max_speed_limit; /*最高速度(km/h)*/
			this->mid_speed_limit= t.mid_speed_limit; /*巡行速度(km/h)*/
			this->min_speed_limit= t.min_speed_limit; /*失速速度(km/h)*/
									...
									if (tt.first == a.first) {
										...
										//ダメージ面に当たった時に装甲値に勝てるかどうか
										{
											...
											if (c.spec.penetration > a.second * (1.0f / std::abs(c.vec.Norm().dot(normal)))) {
												...
												//撃破時エフェクト
												if (veh.HP == 0) {
													set_effect(&t.effcs[ef_bomb], veh.obj.frame(inp.gunframe[0].frame1.first), VGet(0, 0, 0));
												}
												//弾処理
												c.flug= false;
...
public:
	UI() {
		...
		MV1::Load("data/garage/model.mv1", &garage);
	}
	~UI() {
	}
	void select_window(hit::Chara* chara, std::vector<hit::Tanks>* tank, std::vector<hit::Planes>* plane) {
		...
		while (ProcessMessage() == 0) {
			auto old	= chara->vehicle[0].camo_sel;
			...
			SetupCamera_Perspective(deg2rad(90 / 2));
			{
				...
				GetMousePoint(&x, &y);
				if (speed == 0.f) {
					yrad_im= std::clamp(yrad_im + float(m_x - x) / 10.f, -120.f, -30.f);
					xrad_im= std::clamp(xrad_im + float(m_y - y) / 10.f, -5.f, 30.f);
				}
			...
			if (speed == 0.f) {
				campos= (MATRIX_ref::RotY(yrad_m) * MATRIX_ref::RotX(xrad_m)).zvec() * (-15.f) + VGet(0.f, 3.f, 0.f);
			} else {
				easing_set(&campos, VGet(3.25f, 3.f, 7.5f), 0.95f, fps);
			}

			SetCameraPositionAndTargetAndUpVec(campos.get(), (pos + VGet(0.f, 3.f, 0.f)).get(), VGet(0.f, 1.f, 0.f));

			garage.DrawModel();
			{
				wheel_rad-= speed;
				for (auto& v : t[chara->vehicle[0].use_id].wheelframe) {
					t[chara->vehicle[0].use_id].obj.SetFrameLocalMatrix(v.frame.first, MATRIX_ref::RotX(wheel_rad) * MATRIX_ref::Mtrans(v.frame.second));
				}
				for (auto& v : t[chara->vehicle[0].use_id].wheelframe_nospring) {
					t[chara->vehicle[0].use_id].obj.SetFrameLocalMatrix(v.frame.first, MATRIX_ref::RotX(wheel_rad) * MATRIX_ref::Mtrans(v.frame.second));
				}

				t[chara->vehicle[0].use_id].obj.SetMatrix(MATRIX_ref::RotX(deg2rad(speed / (60.f / 3.6f / fps) * 2.f)) * MATRIX_ref::Mtrans(pos));
			...
			if (speed == 0.f) {
				...
				if (ltct == 1) {
					++chara->vehicle[0].camo_sel%= t[chara->vehicle[0].use_id].camog.size();
				}
				if (rtct == 1) {
					if (chara->vehicle[0].camo_sel == 0) {
						chara->vehicle[0].camo_sel= t[chara->vehicle[0].use_id].camog.size() - 1;
					} else {
						--chara->vehicle[0].camo_sel;
					}
				}
			}
			if (t[chara->vehicle[0].use_id].camog.size() > 0) {
				SetDrawScreen(CamScreen.get());
				DrawExtendGraph(0, 0, 240, 240, t[chara->vehicle[0].use_id].camog[chara->vehicle[0].camo_sel], TRUE);
				SetDrawScreen(DX_SCREEN_BACK);

				MV1SetTextureGraphHandle(t[chara->vehicle[0].use_id].obj.get(), t[chara->vehicle[0].use_id].camo_tex, t[chara->vehicle[0].use_id].camog[chara->vehicle[0].camo_sel], TRUE);
			}
			GraphFilter(CamScreen.get(), DX_GRAPH_FILTER_GAUSS, 16, 2400);
			if (chara->vehicle[0].camo_sel != old) {
				if (std::abs(int(int(chara->vehicle[0].camo_sel) - old)) == 1) {
					rad_i+= 360 * int(int(chara->vehicle[0].camo_sel) - old) / int(t[chara->vehicle[0].use_id].camog.size());
				} else {
					if (chara->vehicle[0].camo_sel == 0) {
			...
			SetDrawBlendMode(DX_BLENDMODE_ALPHA, int(255.f * pos.z() / -30.f));
	...
	void draw(const VECTOR_ref& aimpos, const hit::Chara& chara, const bool& ads, const float& fps, const bool& auto_aim, const float& auto_aim_dist, const VECTOR_ref& auto_aim_pos, const float& ratio) {
...

		{
			...
			font18.DrawStringFormat(
				...
				"%d / %d",
				int(chara.vehicle[0].HP),
				int(chara.usetank.HP));
			...
			font18.DrawStringFormat(
				...
				GetColor(255, 255, 255),
				"%d / %d",
				int(chara.vehicle[1].HP),
				int(chara.useplane.HP));
			SetDrawBright(255, 255, 255);
		}
		{
			int j= 0;
...
public:
	...
	void dof(
		...
		if (dof_flag) {
			FarScreen.SetDraw_Screen(far_distance, 6000.0f, fov, campos, camvec, camup);
			{
			...
			NearScreen.SetDraw_Screen(0.01f, near_distance + 0.5f, fov, campos, camvec, camup);
			{
